...
def load_network():
    ...
    pooling_size = 10
    filter_size = 64
    combinations = list(itertools.combinations(range(args.max_n), 2))
    combinations = random.sample(combinations, 45)
    ...
    DENSE_2 = tf.keras.layers.Dense(49, activation="relu")
...
#    print(feature_block.shape)
 #   exit()
    h = tf.keras.layers.Dense(2405, activation="relu")(feature_block)            
    h = tf.keras.layers.Dense(45, activation="relu")(h)            
    h = tf.keras.layers.Flatten()(h)
    output = tf.keras.layers.Dense(1, activation="linear")(h)       
    model = tf.keras.Model(                                         
